...
{
    class T04_Check_ReturnPayments_Monthly_Response : ITestCase
    {
        ...
        public void Run()
        {
            ...
            var urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=10000&NumberOfYears=1";
            ...
            // Veryfing ID values
            {
                ...
                urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=55&TotalAmount=10000&NumberOfYears=1";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.NotFound);

                TestLog.AddMessage(
                    $"Checking response for non existing ID value - index bigger than {ushort.MaxValue}");
                urlToTest =
                    $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID={ushort.MaxValue + 1}&TotalAmount=10000&NumberOfYears=1";
                ...
                urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=-1&TotalAmount=10000&NumberOfYears=1";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);

                TestLog.AddMessage("Checking response for bad ID value - char value");
                urlToTest =
                    $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=test&TotalAmount=10000&NumberOfYears=1";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);
            }

            // Veryfing years value
            {
                TestLog.AddMessage("Checking response for bad years value - value equals 0");
                urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=10000&NumberOfYears=0";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);

                TestLog.AddMessage($"Checking response for bad years value - index bigger than {ushort.MaxValue}");
                urlToTest =
                    $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=10000&NumberOfYears={ushort.MaxValue + 1}";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);

                TestLog.AddMessage("Checking response for bad years value - value lower than 0");
                urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=10000&NumberOfYears=-1";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);

                TestLog.AddMessage("Checking response for bad years value - char value");
                urlToTest =
                    $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=10000&NumberOfYears=test";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);
            }

            // Veryfing amount value
            {
                TestLog.AddMessage("Checking response for bad loan amount value - value equals 0");
                urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=0&NumberOfYears=1";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);

                TestLog.AddMessage("Checking response for bad loan amount value - value lower than 0");
                urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=-1&NumberOfYears=1";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);

                TestLog.AddMessage("Checking response for bad loan amount value - char value");
                urlToTest = $"{ServerAddress}/api/Loan/ReturnPayments?LoanTypeID=0&TotalAmount=test&NumberOfYears=1";
                WebHelpers.VerifyEndpoints(urlToTest, HttpStatusCode.BadRequest);
            }
        }
